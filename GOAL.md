...
以下流程是 Sextant 记忆系统的唯一主流程。其他文档里的 Mermaid 图只展开其中某个局部片段，不定义第二套全局流程。

```mermaid
flowchart TD
    A[Raw Source\n手稿 / 原著 / 设定集 / 角色卡] --> B[Source Preservation\n保存原始材料与版本]
    B --> C[Source Normalization\n生成 ProcessedMarkdownView]
    ...
    M -->|低风险 / 已接受| N[Canon Promotion\n允许改写 Current Canon]
    M -->|中高风险| O[ReviewItem\n保留证据但不自动升格]
    N --> P[Memory Pages\nCurrent Canon / Open Threads]
    L --> P
    P --> Q[Story Auto-Link\n确定性建图]
    Q --> R[Graph Projection\n可重建故事图谱]
    P --> S[Retrieval & Context Pack\n问答 / 续写上下文 / 查矛盾]
    R --> S
```

详细说明见：[01-data-flow.md](goals/01-data-flow.md)、[16-source-normalization.md](goals/16-source-normalization.md)、[15-event-aggregation.md](goals/15-event-aggregation.md)、[17-incremental-memory-writeback.md](goals/17-incremental-memory-writeback.md)、[18-conflict-policy.md](goals/18-conflict-policy.md)、[19-story-auto-link.md](goals/19-story-auto-link.md)。

## 4. 机制概览与术语索引

| 概念 | 在主流程中的位置 | 作用 | 详细文档 |
|---|---|---|---|
| Story Skills | 横跨结构解析、抽取、聚合、回写、检查、问答 | 把导入、抽取、聚合、回写、检查拆成可审计的处理协议 | [13-skills-and-resolver.md](goals/13-skills-and-resolver.md) |
...
| Story Schema Pack | 约束 Mention、EventCandidate、FactAssertion、GraphProjection | 限定实体、事件、关系类型，让规则优先和题材扩展可以共存 | [14-story-schema-packs.md](goals/14-story-schema-packs.md) |
...
| Evidence / Log Writeback | FactAssertion 之后 | 新证据先进入 Appearance Log、Event Log、Source refs，不等于 Current Canon 改写 | [17-incremental-memory-writeback.md](goals/17-incremental-memory-writeback.md) |
| Conflict Policy Gate | FactAssertion 到 Canon Promotion 之间 | 阻断高风险事实、别名或状态变化的自动升格 | [18-conflict-policy.md](goals/18-conflict-policy.md) |
| ReviewItem | Conflict Policy Gate 输出 | 把别名、冲突、连续性和 canon promotion 风险变成作者可处理事项 | [18-conflict-policy.md](goals/18-conflict-policy.md) |
| MemoryPage | Canon Promotion 之后 | 面向作者和 AI 的 Current Canon、日志、开放问题与证据汇总 | [07-memory-pages.md](goals/07-memory-pages.md) |
...
| ContextPack | 用户请求问答或续写时按需生成 | 使用最新 MemoryPage、GraphProjection、ReviewItem 生成上下文包 | [09-retrieval-context-pack.md](goals/09-retrieval-context-pack.md)、[17-incremental-memory-writeback.md](goals/17-incremental-memory-writeback.md) |
...
| SourceVersion | RawSource 的版本记录 | 否 | 间接引用 | [02-core-data-structures.md](goals/02-core-data-structures.md) |
...
| EvidenceLogEntry | 可先写入的证据/出场/事件日志 | 否 | 是 | [17-incremental-memory-writeback.md](goals/17-incremental-memory-writeback.md) |
...
| ContextPack | 续写或问答时按需生成的上下文包 | 否 | 是 | [09-retrieval-context-pack.md](goals/09-retrieval-context-pack.md) |
| ReviewItem | 冲突、连续性、别名、canon promotion 风险提示 | 否 | 是 | [18-conflict-policy.md](goals/18-conflict-policy.md) |
...
Evidence / Log 先写入
  ↓
Conflict Policy Gate 决定是否 Canon Promotion
  ↓
MemoryPage 维护 Current Canon
  ↓
Story Auto-Link 确定性投影图谱
  ↓
Review Policy 保护 canon，不阻断创作
```
