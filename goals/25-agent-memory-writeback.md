...
sequenceDiagram
    ...
    Agent->>Draft: 生成候选文本
    Draft->>Author: 提供候选和风险说明
    Author->>Draft: 接受 / 修改 / 拒绝
    Draft->>Delta: 接受后转成 SourceDelta
...
| review_required | 有风险，生成 ReviewItem |
| rejected_from_canon | 保留证据，但不进入 Current Canon |

## 6. Agent 写作后的 ReviewItem

Agent Review 和 Memory Conflict Policy 都可能产生 ReviewItem。

| 来源 | 作用 |
|---|---|
| Agent Review | 在作者接受前提示候选文本风险 |
| Memory Conflict Policy | 在接受文本进入 Memory 后检查 canon promotion 风险 |
